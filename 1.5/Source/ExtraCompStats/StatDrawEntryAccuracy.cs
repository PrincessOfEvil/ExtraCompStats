...
namespace ExtraStats
    ...
    public class StatDrawEntryAccuracy : StatDrawEntry
		...
		private static void ResetAccuracyCache()
			{
			renderedStats = new Dictionary<(ThingDef, QualityCategory, int), Texture2D>(); 
			}
        ...
        public static float Render(float currentY, Rect windowRect, Thing thing, StatDrawEntry statIn)
            ...
            if (thing != null && thing.TryGetVerb() != null && statIn is StatDrawEntryAccuracy stat)
                ...
                if (!renderedStats.TryGetValue((thing.def, quality, stat.GetTypeHash()), out Texture2D texture))
                    {
                ...
                if (stat.goesBeyond() && Mouse.IsOver(baRect))
                    ...
                    Widgets.Label(barBeyond.width / (float)beyond - margin, ref currentY, largeMargin, stat.label(1));

                    if (Input.GetMouseButtonUp(0)) ResetAccuracyCache();
                    }
                else
                ...
                if (Mouse.IsOver(texRect))
                    ...
                    var rectLevel = new Rect(drawRect)
                        {
                        x = smallMargin,
                        width = margin - smallMargin + texture.width * StatDrawEntryAccuracy.scale
                        };
                    var rectAcc = new Rect(drawRect)
                        {
                        y = currentY - margin + smallMargin,
                        height = margin - smallMargin + texture.height * StatDrawEntryAccuracy.scale
                        };
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il)
            ...
            foreach (var instruction in instructions)
                ...
                if (instruction.opcode == OpCodes.Stsfld &&
                    instruction.operand is FieldInfo {Name: "rightPanelHeight"})
                    {
